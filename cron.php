...
if ($_GET['do'] == 'settle') {
    if ($conf['settle_open'] == 1 || $conf['settle_open'] == 3) {
        ...
        foreach ($rows as $k=>$v){
            ...
            $rate = ($point['c'][0]) * (1 + 0.02);//此处设置USDT汇率上浮值0.02代表上浮2%
            // $rate = ($point['c'][0]) * 1;
            $usdt = round($realmoney / $rate, 2);

            if ($DB->exec("INSERT INTO `pre_settle` (`uid`, `type`, `username`, `account`, `money`, `realmoney`, `usdt`,`addtime`, `status`) VALUES ('{$v['uid']}', '{$v['settle_id']}', '{$v['username']}', '{$v['account']}', '{$v['money']}', '{$realmoney}','{$usdt}', '{$date}', '0')")) {
                changeUserMoney($v['uid'], $v['money'], false, '自动结算');
                $allmoney += $realmoney;
            }
        }
...
//            $point = array_pop($points);
//
//            $rate = ($point['c'][0]) * (1 + 0.02);
//            $usdt = round($realmoney / $rate, 2);
//
//            if ($DB->exec("INSERT INTO `pre_settle` (`uid`, `type`, `username`, `account`, `money`, `realmoney`, `usdt`,`addtime`, `status`) VALUES ('{$row['uid']}', '{$row['settle_id']}', '{$row['username']}', '{$row['account']}', '{$row['money']}', '{$realmoney}','{$usdt}', '{$date}', '0')")) {
