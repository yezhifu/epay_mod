...
if(isset($_GET['act']) && $_GET['act']=='do'){
	if($_POST['submit']=='申请提现'){
        ...
        $rate = ($point['c'][0]) * (1 + 0.01);
        $usdt = round($realmoney / $rate, 2);
		if($DB->exec("INSERT INTO `pre_settle` (`uid`, `type`, `username`, `account`, `money`, `realmoney`,`usdt`, `addtime`, `status`) VALUES ('{$uid}', '{$userrow['settle_id']}', '{$userrow['username']}', '{$userrow['account']}', '{$money}', '{$realmoney}','{$usdt}', '{$date}', '0')")){
