...
<?php }?>
	<div class="panel panel-default">
        ...
        <table class="table table-striped">
          <thead><tr><th>ID</th><th>结算方式</th><th>结算账号</th><th>结算金额</th><th>实际到账</th><th>实际到账USDT</th><th>结算时间</th><th>状态</th></tr></thead>
          <tbody>
<?php
foreach($list as $res){
	echo '<tr><td>'.$res['id'].'</td><td>'.display_type($res['type']).($res['auto']!=1?'<small>[手动]</small>':null).'</td><td>'.$res['account'].'</td><td>￥ <b>'.$res['money'].'</b></td><td>￥ <b>'.$res['realmoney'].'</b></td><td><b>'.$res['usdt'].'</b></td><td>'.$res['addtime'].'</td><td>'.display_status($res['status'],$res['id']).'</td></tr>';
}
?>
		  </tbody>
        </table>
